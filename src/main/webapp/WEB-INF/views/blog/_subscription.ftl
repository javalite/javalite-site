<div class="p-t-15">
    <div class="alert bs-alert-old-docs p-b-0">
        <form action="https://expresspigeon.com/subscription/add_contact" method="post" class="row">
            <input type="hidden" name="guid" value="d8a9afc9-bab3-454a-a7e3-f0f84a45c6c5"/>
            <div class="col-xs-12 col-sm-4 m-b-15">
                <input name="email" placeholder="Email" type="email"  class="required form-control" value="" />
            </div>
            <div class="col-xs-12 col-sm-4 m-b-15">
                <input placeholder="First name" class="form-control" name="first_name"/>
            </div>
            <div class="col-xs-12 col-sm-4 m-b-15">
                <button type="submit" class="btn bgm-black btn-block" href="#">Join our Newsletter</button>
            </div>
        </form>
    </div>
</div>
